...
Shape ShapeGenerator::Teapot(uint tesselation, const glm::mat4& lidTransform) {
	Shape ret;
...
Shape ShapeGenerator::Torus(uint tesselation)
{
	Shape ret;
	...
	for (uint round1 = 0; round1 < tesselation; round1++)
	{
		...
		{
			Vertex3D& v = ret.vertices[round1 * tesselation + round2];
			glm::vec4 glmVert(
	...
	Shape ret2 = MakePlaneUnseamedIndices(tesselation);
	ret.numIndices = ret2.numIndices;
	ret.indices = ret2.indices;
	return ret;
}

Shape ShapeGenerator::Sphere(uint tesselation)
{
	Shape ret = MakePlaneVertices(tesselation);
	Shape ret2 = MakePlaneIndices(tesselation);
	ret.indices = ret2.indices;
	ret.numIndices = ret2.numIndices;
	...
	for (size_t col = 0; col < dimensions; col++)
	{
		...
		for (size_t row = 0; row < dimensions; row++)
		{
			...
			Vertex3D& v = ret.vertices[vertIndex];
...
Shape ShapeGenerator::GenerateNormals(const Shape& data)
{
	Shape ret;
	...
	{
		uint vertIndex = i * 2;
		Vertex3D& v1 = ret.vertices[vertIndex];
		Vertex3D& v2 = ret.vertices[vertIndex + 1];
		const Vertex3D& sourceVertex = data.vertices[i];
...
Shape ShapeGenerator::MakePlaneUnseamedIndices(uint tesselation)
{
	Shape ret;
	uint dimensions = tesselation * tesselation;
